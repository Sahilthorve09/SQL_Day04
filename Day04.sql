...
-- Q2
select * from cars where mileage = (select max(Mileage) from cars);

-- Q3
select * from cars where price > (select avg(price) from cars where fueltype = 'gasoline') ;

-- Q4
select brand from cars group by brand having count(*) = (select count(*) from cars group by brand order by count(*) desc limit 1);